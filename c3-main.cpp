...
// Function for ICP scan matching
Eigen:: Matrix4d ICP(PointCloudT::Ptr target, PointCloudT::Ptr source, Pose startingPose, int iterations){
	//Define a rotation and translation matrix
  	Eigen::Matrix4d transformation_matrix = Eigen::Matrix4d::Identity();
  	
  	//Start pose to be aligned with source
  	...
  	// Declaring some pre-defined parameters 
  	...
  	//ICP ouptput point cloud and align.
	...
	while (!viewer->wasStopped())
  	{
		...
		if(!new_scan){
			...
			// TODO: (Filter scan using voxel filter)
          	...
          	//Perform Filtering to get the filtered cloud
            vg.filter(*cloudFiltered);
              

			// TODO: Find pose transform by using ICP or NDT matching
                      
          	//Using ICP to find pose transform
          	Eigen::Matrix4d transform = ICP(mapCloud, cloudFiltered, pose, 40);
          
          	// update pose based on the transform
			pose = getPose(transform);

			// TODO: Transform scan so it aligns with ego's actual pose and render that scan
            // Corrected scan declaration
            PointCloudT::Ptr scanCorrected (new PointCloudT);
          	
          	// filtered cloud transformation into scan which is corrected.
